...
# 1. 資料加載
# file_path = "C:/Users/Administrator/Desktop/1553768847-housing.csv"
...
# 排除 ocean_proximity_NEAR BAY 和 ocean_proximity_NEAR OCEAN
exclude_features = ["ocean_proximity_NEAR BAY", "ocean_proximity_NEAR OCEAN"]
selected_features = [feature for feature in selected_features if feature not in exclude_features]
selected_features_idx = [features.columns.get_loc(feature) for feature in selected_features]
...
for name, model in models.items():
    ...
    if 'transformer' in locals():  # 如果使用了 Yeo-Johnson
        y_test_inverse = transformer.inverse_transform(y_test.reshape(-1, 1)).flatten()
        y_pred_inverse = transformer.inverse_transform(y_pred.reshape(-1, 1)).flatten()
    else:  # 如果使用了 Box-Cox
        y_test_inverse = np.exp(y_test) if lambda_value == 0 else np.power(y_test * lambda_value + 1, 1 / lambda_value)
        y_pred_inverse = np.exp(y_pred) if lambda_value == 0 else np.power(y_pred * lambda_value + 1, 1 / lambda_value)

    mae_original = mean_absolute_error(y_test_inverse, y_pred_inverse)

    results[name] = {
        "MAE": mae,
        "R2": r2,
        "MAE (Original Scale)": mae_original
    }
...
for model_name, metrics in results.items():
    ...
    mae_original_formatted = f"{metrics['MAE (Original Scale)']:.2f}"
    print(f"{model_name}: MAE = {mae_formatted}, R² = {r2_formatted}, MAE (Original Scale) = ${mae_original_formatted}")
